			...
			<div
				...
				<div class="btn-toolbar mb-2 mb-md-0">
					<div class="btn-group mr-2">
						<form action="AjouterProjet" method="GET">
				...
				<table class="table table-striped table-sm">
					...
					<tbody>
						<c:forEach items="${requestScope.projets}" var="projet">
							<tr>
								<td><c:out value="${projet.id}"></c:out></td>
								<td><c:out value="${projet.intitule}"></c:out></td>
								<td><c:out value="${projet.description}"></c:out></td>
								<td><c:out value="${projet.chargeHoraire}"></c:out></td>
								<td><c:out value="${projet.statut.getStatutName()}"></c:out></td>
								<td><c:url value="/ModifierProjet" var="modifierProjet">
										<c:param name="id" value="${projet.id }" />
									</c:url> <a href="${modifierProjet }">Gérer</a></td>
								<td><c:url value="/AfficherTacheProjet" var="afficherTacheProjet">
										<c:param name="id" value="${projet.id }" />
									</c:url> <a href="${afficherTacheProjet }">Liste des taches</a></td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
